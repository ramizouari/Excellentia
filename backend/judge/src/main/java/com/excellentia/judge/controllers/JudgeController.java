...
import com.excellentia.judge.configuration.OtelConfiguration;
...
@Timed(value="judge.time", description = "Time taken to judge a solution")
public class JudgeController
{
                      ...
                      HttpServletRequest request) throws IOException
    {
        ...
        try(Tracer.SpanInScope ws = tracer.withSpan(span.start()))
        {
            ...
            span.event("Sending submission file to compiler.");
            ...
            span.event("Compiler returned run id.");
            ...
            logger.atDebug().setMessage("Sending submission file to runner")
                    ...
                    .log();
            span.event("Sending run id to runner. id=%s".formatted(run.get("id")));
            response= runRequest.postForObject(discoveryConfiguration.getRunner().getUrl().concat("/runner"), runRequestEntity, Map.class);
            logger.atDebug().setMessage("Received response {}.")
                    ...
                    .addKeyValue("problem_id",problemId)
                    .log();
            span.event("Received verdict from runner. id=%s".formatted(run.get("id")));
        }
        finally {
            span.end();
        }
